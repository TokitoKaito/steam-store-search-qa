# BUG-003 — After a server error, search on the results page stops working and is not restored by refreshing the page
...
3. In the search field, enter a single double quote: " — and press Enter. A server error is displayed (BUG-001)
...
Step 4 — the new query is not processed: the same server error as in step 3 is returned;
...
- **Priority: Medium** — the defect is only reachable through BUG-001, but it concerns error-response handling in general and will appear on any other server error on this page.
