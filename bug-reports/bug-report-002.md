# BUG-002 — Site header is duplicated inside the search results area when the search request fails
...
- **Priority: Medium** — the defect is visible to users on a public store page and looks like broken layout. It also presumably points to a cause beyond cosmetics: judging by the observed result, the error response is inserted into the results container in full, without being processed. The same defect will appear on any other failed response, not only the one described in BUG-001.
